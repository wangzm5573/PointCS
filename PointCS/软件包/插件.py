...
def 暂停执行并检测(总秒数: 联合[整数, 浮点数], 检测间隔秒数: 联合[整数, 浮点数] = 0.1, 检测函数: 可调用[[], 布尔值] = lambda: False) -> 布尔值:
    if 检测间隔秒数 >= 总秒数:
        raise 值不合法("检测间隔秒数应该小于总秒数.")
    开始时刻 = 时间.性能计时器()
...
class 租赁服(对象):
    ...
    def _读取速建者输出(当前对象: 租赁服, 读到此字符串时退出: 可选[字符串] = 无, 崩溃重启: 布尔值 = 假, 可停止线程对象: 可选[可停止线程] = 无) -> 无:
        线程是否正在停止 = lambda: 假 if (可停止线程对象 is None) else 可停止线程对象.正在停止
        错误信息 = 字符串("")
        异常类型 = 速建者启动时异常
        while not 线程是否正在停止():
