...
class 速建者用户中心操作(对象):
    ...
    def _网络请求(当前对象: 速建者用户中心操作, 速建者用户中心应用程序编程接口的统一资源定位符: 字符串, 请求方式: 字面["获取", "提交"], 是否JSON化返回结果: 布尔值 = 真, 是否在不成功时引发异常: 布尔值 = 真) -> 联合[JSON, 字节串]:
        if 请求方式 == "获取":
            请求函数 = 当前对象.会话.获取
        elif 请求方式 == "提交":
            请求函数 = 当前对象.会话.提交
        ...
        if 时间.时间() -当前对象.会话创建时间 >= 600:
            当前对象._登录用户中心()
        返回结果 = 请求函数(速建者用户中心应用程序编程接口的统一资源定位符)
        if not 是否JSON化返回结果:
            return 返回结果.原始内容
        返回结果 = 返回结果.JSON内容()
        if 是否在不成功时引发异常:
    ...
    def _获取(当前对象: 速建者用户中心操作, 速建者用户中心应用程序编程接口的统一资源定位符: 字符串, 是否JSON化返回结果: 布尔值 = 真, 是否在不成功时引发异常: 布尔值 = 真) -> 联合[JSON, 字节串]:
        返回结果 = 当前对象._网络请求(速建者用户中心应用程序编程接口的统一资源定位符, "获取", 是否JSON化返回结果, 是否在不成功时引发异常)
        return 返回结果


    def _提交(当前对象: 速建者用户中心操作, 速建者用户中心应用程序编程接口的统一资源定位符: 字符串, 是否JSON化返回结果: 布尔值 = 真, 是否在不成功时引发异常: 布尔值 = 真) -> 联合[JSON, 字节串]:
        返回结果 = 当前对象._网络请求(速建者用户中心应用程序编程接口的统一资源定位符, "提交", 是否JSON化返回结果, 是否在不成功时引发异常)
        return 返回结果
    ...
    def _修改槽位租赁服号(当前对象: 速建者用户中心操作, 槽位编号: 字符串, 租赁服号: 字符串, 是否无需更新详细信息: 布尔值 = 假) -> 无:
        if (not 租赁服号.是否为十进制整数()) or (整数(租赁服号) <= 0):
            raise 值不合法("修改槽位租赁服号异常: 参数 租赁服号 应为正整数.")
        if not 是否无需更新详细信息:
            当前对象._获取用户详细信息()
